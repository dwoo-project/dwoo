...
class FuncTests extends PHPUnit_Framework_TestCase
{
    ...
    {
        // test simple assign
        $tpl = new Dwoo\Template\String('{assign "bar" foo}{$foo}');
        $tpl->forceCompilation();

        $tpl2 = new Dwoo\Template\String('{assign baz foo}{$foo}');
        ...
        $tpl = new Dwoo\Template\String('{assign reverse($foo) foo}{foreach $foo val}{$val}{/foreach}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{capitalize "hello world 1st"}-{capitalize "hello world 1st" true}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{cat 3 bar "FOO"}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{counter start=0 skip=2}{counter}{counter}{counter}');
        $tpl->forceCompilation();

        $this->assertEquals('0246', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{counter start=0 skip=2 assign=foo}{counter}{counter}{counter}{$foo}');
        $tpl->forceCompilation();

        $this->assertEquals('6', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{counter start=0 skip=2 assign=foo direction=down}{counter}{counter}{counter}{$foo}');
        $tpl->forceCompilation();

        $this->assertEquals('-6', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{counter start=0 skip=3 print=false}{counter}{counter}{counter}{counter print=true}');
        $tpl->forceCompilation();

        $this->assertEquals('12', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{counter start=0 skip=3 print=false}{counter skip=1 start=10 assign=moo direction=down}{$moo}/{counter}{$moo}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{count_characters "hello world"}{count_characters "hello world" true}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{count_paragraphs $foo}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{count_sentences $foo}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{count_words $foo}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{cycle "hoy" array(foo,bar) }{cycle hoy}{cycle hoy}{cycle name=hoy print=false}{cycle name="hoy"}{cycle name=hoy reset=true advance=false}{cycle name="hoy" advance=true}');
        $tpl->forceCompilation();

        $this->assertEquals('foobarfoofoofoofoo', $this->dwoo->get($tpl, array('foo' => "sfsdf.smdf\nmkoep\tsdlk sdfsdf"), $this->compiler));

        $tpl = new Dwoo\Template\String('{cycle "hoy" "foo,bar"}{cycle hoy}{cycle hoy assign="moo"}|{$moo}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{date_format $dwoo.now "%Y%H:%M:%S"}{date_format $foo "%Y%H:%M:%S" "one hour ago"}{date_format ""}');
        $tpl->forceCompilation();
        ...
        $tpl = new Dwoo\Template\String('{date_format "'.date('Y-m-d H:i:s', $_SERVER['REQUEST_TIME'] - 10).'" "%Y:%H:%M:%S"}');
        $tpl->forceCompilation();
        $this->assertEquals(strftime('%Y:%H:%M:%S', $_SERVER['REQUEST_TIME'] - 10), $this->dwoo->get($tpl, array('foo' => ''), $this->compiler));

        $tpl = new Dwoo\Template\String('{date_format "" "%Y%H:%M:%S" '.($_SERVER['REQUEST_TIME'] - 10).'}');
        $tpl->forceCompilation();
        $this->assertEquals(strftime('%Y%H:%M:%S', $_SERVER['REQUEST_TIME'] - 10), $this->dwoo->get($tpl, array('foo' => ''), $this->compiler));
    }
    ...
    {
        $tpl = new Dwoo\Template\String('{default $foo bar}{default $foo2 bar}{default $foo3 bar}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{do assign($foo bar)}{do $bar}{$bar}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{do}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{eol}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{escape $foo}');
        $tpl->forceCompilation();

        $this->assertEquals('&quot;', $this->dwoo->get($tpl, array('foo' => '"'), $this->compiler));

        $tpl = new Dwoo\Template\String('{escape $foo htmlall}');
        $tpl->forceCompilation();

        $this->assertEquals('&eacute;', $this->dwoo->get($tpl, array('foo' => 'é'), $this->compiler));

        $tpl = new Dwoo\Template\String('{escape $foo htmlall iso-8859-1}');
        $tpl->forceCompilation();

        $this->assertEquals('&uuml;', $this->dwoo->get($tpl, array('foo' => utf8_decode('ü')), $this->compiler));

        $tpl = new Dwoo\Template\String('{escape $foo url}');
        $tpl->forceCompilation();

        $this->assertEquals(rawurlencode(':#?/'), $this->dwoo->get($tpl, array('foo' => ':#?/'), $this->compiler));

        $tpl = new Dwoo\Template\String('{escape $foo quotes}');
        $tpl->forceCompilation();

        $this->assertEquals("\\'", $this->dwoo->get($tpl, array('foo' => '\''), $this->compiler));

        $tpl = new Dwoo\Template\String('{escape $foo urlpathinfo}');
        $tpl->forceCompilation();

        $this->assertEquals(rawurlencode(':#?').'/', $this->dwoo->get($tpl, array('foo' => ':#?/'), $this->compiler));

        $tpl = new Dwoo\Template\String('{escape $foo hex}');
        $tpl->forceCompilation();

        $this->assertEquals('%0a', $this->dwoo->get($tpl, array('foo' => chr(10)), $this->compiler));

        $tpl = new Dwoo\Template\String('{escape $foo hexentity}');
        $tpl->forceCompilation();

        $this->assertEquals('&#x0a;', $this->dwoo->get($tpl, array('foo' => chr(10)), $this->compiler));

        $tpl = new Dwoo\Template\String('{escape $foo javascript}');
        $tpl->forceCompilation();

        $this->assertEquals('\\\\n', $this->dwoo->get($tpl, array('foo' => '\\n'), $this->compiler));

        $tpl = new Dwoo\Template\String('{escape $foo mail}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{eval $foo}{assign "baz" test}{eval $foo bar}+{$bar}{eval ""}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\File(TEST_DIRECTORY.'/resources/extends/sub/test.html');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\File(TEST_DIRECTORY.'/resources/extends_braces/sub/test.html');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\File(TEST_DIRECTORY.'/resources/extends/root.html');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{fetch file="'.TEST_DIRECTORY.'/resources/test.html"}{fetch ""}{fetch file="'.TEST_DIRECTORY.'/resources/test.html" assign="boo"}|{$boo}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{include file=\'file:'.TEST_DIRECTORY.'/resources/test.html\' foo=$a bar=$b}');
        $tpl->forceCompilation();

        $this->assertEquals('AB', $this->dwoo->get($tpl, array('a' => 'A', 'b' => 'B')));

        $tpl = new Dwoo\Template\String('{include file="" foo=$a bar=$b}');
        $tpl->forceCompilation();
        $this->assertEquals('', $this->dwoo->get($tpl, array('a' => 'A', 'b' => 'B')));

        $tpl = new Dwoo\Template\String('{include file=\'file:'.TEST_DIRECTORY.'/resources/test.html\'}');
        $tpl->forceCompilation();

        $this->assertEquals('ab', $this->dwoo->get($tpl, array('foo' => 'a', 'bar' => 'b'), $this->compiler));

        $tpl = new Dwoo\Template\File(TEST_DIRECTORY.'/resources/inctest.html');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{include file=$tpl foo=$a bar=$b}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\File(TEST_DIRECTORY.'/resources/subfolder/inctest.html');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{indent $foo 6 "-"}');
        $tpl->forceCompilation();
...
//    }
    ...
    {
        $tpl = new Dwoo\Template\String('{lower "FOO"}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{mailto address="me@example.com" encode="jschar"}');
        $tpl->forceCompilation();
        ...
        $tpl = new Dwoo\Template\String('{mailto address="me@example.com" text="send me some mail"}');
        $tpl->forceCompilation();

        $this->assertEquals('<a href="mailto:me@example.com" >send me some mail</a>', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{mailto address="me@example.com" encode="javascript"}');
        $tpl->forceCompilation();
        ...
        $tpl = new Dwoo\Template\String('{mailto address="me@example.com" encode="hex"}');
        $tpl->forceCompilation();
        ...
        $tpl = new Dwoo\Template\String('{mailto address="me@example.com" subject="Hello to you!" cc="you@example.com,they@example.com" extra=\'class="email"\'}');
        $tpl->forceCompilation();
        ...
        $tpl = new Dwoo\Template\String('{mailto address="" subject="Hello to you!" cc="you@example.com,they@example.com" extra=\'class="email"\'}');
        $tpl->forceCompilation();
        $this->assertEquals('', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{mailto address="me@example.com" subject="Hello to you!" bcc="moo" newsgroups="bleh" followupto="meh"}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{nl2br "f
a"}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{regex_replace "FOOMOO" "/OO\$/" "L"}');
        $tpl->forceCompilation();

        $this->assertEquals('FOOML', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{regex_replace "FOOMOO" "/OO\$/e" "die"}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{replace "FOOMOO" "OO" "L"}');
        $tpl->forceCompilation();

        $this->assertEquals('FLML', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{replace "FOOMOO" "oo" "l" off}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{return bar=$foo}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{include cat("file:" %TEST_DIRECTORY "/resources/returnTest.html") val="foo"}{$bar}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{reverse "abc"}');
        $tpl->forceCompilation();

        $this->assertEquals('cba', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{foreach reverse(array(a,b,c)) foo}{$foo}{/foreach}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{spacify "ABC" "+"}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{string_format 53.3942 "%.2f"}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{strip_tags "<a href=\'foo\'>test</a>test"}');
        $tpl->forceCompilation();

        $this->assertEquals(' test test', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{strip_tags "<a href=\'foo\'>test</a>test" false}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{whitespace "a		b	 c   		   d"}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{tif $a && $b && 3==3 ? foo : bar}');
        $tpl->forceCompilation();
        ...
        $tpl = new Dwoo\Template\String('{tif $a foo bar}');
        $tpl->forceCompilation();
        ...
        $tpl = new Dwoo\Template\String('{tif $a ? foo}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{truncate "abcdefghijklmnopqrstuvwxyz" 20 "..." true}');
        $tpl->forceCompilation();
        $this->assertEquals('abcdefghijklmnopq...', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{truncate "abcdefghijklmnopqrstuvwxyz" 20 "..." true true}');
        $tpl->forceCompilation();
        $this->assertEquals('abcdefghi...stuvwxyz', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{truncate "" 20 "..." true true}');
        $tpl->forceCompilation();
        $this->assertEquals('', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{truncate "a" 20 "..." true true}');
        $tpl->forceCompilation();
        $this->assertEquals('a', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{truncate "abcdefghijklmnopqrstuvwxyz abcdef" 20 "..."}');
        $tpl->forceCompilation();
        $this->assertEquals('abcdefghijklmnopq...', $this->dwoo->get($tpl, array(), $this->compiler));

        $tpl = new Dwoo\Template\String('{truncate "abc abc abc abc abc abc" 10 "..."}');
        $tpl->forceCompilation();
    ...
    {
        $tpl = new Dwoo\Template\String('{wordwrap "abcdefghijklmnopqrstuvwxyz" 8 "\n" true}');
        $tpl->forceCompilation();
