...
define('DWOO_PATH', dirname(__FILE__).DIRECTORY_SEPARATOR);

// include class paths or rebuild paths if the cache file isn't there
if(!(file_exists(DWOO_PATH.'cache'.DIRECTORY_SEPARATOR.'classpath.cache') && include DWOO_PATH.'cache'.DIRECTORY_SEPARATOR.'classpath.cache'))
...
class Dwoo
{
     ...
     *
     * defaults to DWOO_PATH/compiled
     *
     * @var string
     */
    ...
    {
        $this->cacheDir = DWOO_PATH.'cache'.DIRECTORY_SEPARATOR;
        $this->compileDir = DWOO_PATH.'compiled'.DIRECTORY_SEPARATOR;
    }
    ...
    public function output($tpl, $data = array(), DwooICompiler $compiler = null, $return = false)
    {
        ...
        elseif(is_string($tpl))
            ...
            throw new Exception('Template must be a DwooITemplate (i.e. DwooTemplateFile) or a valid path to a template', E_USER_NOTICE);
        ...
        elseif(is_array($data))
            ...
            throw new Exception('Data must be a DwooIDataProvider object (i.e. DwooData) or an associative array', E_USER_NOTICE);
    ...
    public function addPlugin($name, $callback)
    {
        ...
        {
            throw new Exception('Callback could not be processed correctly, please check that the function/class you used exists.', E_USER_NOTICE);
        }
    }
    ...
    public function addResource($name, $class, $compilerFactory = array('DwooCompiler', 'compilerFactory'))
    {
        ...
        if(in_array('DwooITemplate', $interfaces) === false)
            throw new Exception('The resource class must implement DwooITemplate', E_USER_NOTICE);
...
class DwooLoader
{
    ...
    /**
     * rebuilds class paths, scans the given directory recursively and saves all paths in the outputPath/classpath.cache file
     *
     * @param string $path the plugin path to scan
     * @param string $outputPath the path where to store the plugin paths cache, directory must be writable
     * @access protected
     */
    public static function rebuildClassPathCache($path, $outputPath)
    {
        if($outputPath!==false)
        {
            $tmp = self::$classpath;
        ...
        if($outputPath!==false)
        {
            if(!file_put_contents($outputPath.DIRECTORY_SEPARATOR.'classpath.cache', '<?php DwooLoader::$classpath = '.var_export(self::$classpath, true).' + DwooLoader::$classpath; ?>'))
                throw new Exception('Dwoo could not write into '.$outputPath.', either because the folder is not there (create it) or because of the chmod configuration (please ensure this directory is writable by php)');
            self::$classpath += $tmp;
        }
    }
    ...
    public static function loadPlugin($class)
    {
        ...
        {
            self::rebuildClassPathCache(DWOO_PATH.'plugins', DWOO_PATH.'cache');
            foreach(self::$paths as $path)
                self::rebuildClassPathCache($path, $path);
     ...
     * @param string $writeableDir the path where to store the plugin paths cache, directory must be writable
     */
    public static function addDirectory($dir, $writeableDir=null)
    {
        if(array_search($dir, self::$paths, true) !== false)
            return;
        if($writeableDir === null)
            $writeableDir = $dir;
        self::$paths[] = $dir;
        if(file_exists($writeableDir.DIRECTORY_SEPARATOR.'classpath.cache'))
            include $writeableDir.DIRECTORY_SEPARATOR.'classpath.cache';
        else
