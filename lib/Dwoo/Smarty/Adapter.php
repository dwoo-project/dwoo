...
if (class_exists('\Dwoo\Compiler') === false) {
...
class Adapter extends Core {
	...
	public function __set($p, $v) {
		...
		if (array_key_exists($p, $this->compat['properties']) !== false) {
			if ($this->show_compat_errors) {
				$this->triggerError('Property ' . $p . ' is not available in the Adapter, however it might be implemented in the future, check out http://wiki.dwoo.org/index.php/SmartySupport for more details.', E_USER_NOTICE);
			}
			$this->compat['properties'][$p] = $v;
		}
		else {
			if ($this->show_compat_errors) {
				$this->triggerError('Property ' . $p . ' is not available in the Adapter, but it is not listed as such, so you might want to tell me about it at j.boggiano@seld.be', E_USER_NOTICE);
			}
		}
	}
	...
	public function __call($m, $a) {
		...
		elseif ($this->show_compat_errors) {
			if (array_search($m, $this->compat['methods']) !== false) {
				$this->triggerError('Method ' . $m . ' is not available in the Adapter, however it might be implemented in the future, check out http://wiki.dwoo.org/index.php/SmartySupport for more details.', E_USER_NOTICE);
			}
			else {
				$this->triggerError('Method ' . $m . ' is not available in the Adapter, but it is not listed as such, so you might want to tell me about it at j.boggiano@seld.be', E_USER_NOTICE);
			}
		}
	}
	...
	protected $compat = array(
		'methods'       => array(
			'register_resource', 'unregister_resource', 'load_filter', 'clear_compiled_tpl', 'clear_config', 'get_config_vars', 'config_load'
		), 'properties' => array(
			'cache_handler_func' => null, 'debugging' => false, 'error_reporting' => null, 'debugging_ctrl' => 'NONE', 'request_vars_order' => 'EGPCS', 'request_use_auto_globals' => true, 'use_sub_dirs' => false, 'autoload_filters' => array(), 'default_template_handler_func' => '', 'debug_tpl' => '', 'cache_modified_check' => false, 'default_modifiers' => array(), 'default_resource_type' => 'file', 'config_overwrite' => true, 'config_booleanize' => true, 'config_read_hidden' => false, 'config_fix_newlines' => true, 'config_class' => 'Config_File',
		),
	);
	...
	public $security_settings = array(
		'PHP_HANDLING'   => false, 'IF_FUNCS' => array(
			'list', 'empty', 'count', 'sizeof', 'in_array', 'is_array',
	...
	public function register_prefilter($callback) {
		$processor = new \Dwoo\Smarty\Processor\Adapter($this->compiler);
	...
	public function register_outputfilter($callback) {
		$filter = new \Dwoo\Smarty\Filter\Adapter($this);
	...
	protected function makeTemplate($file, $cacheId, $compileId) {
		...
		if (!isset(self::$tplCache[$hash])) {
			...
			if (substr($file, 0, 1) === '/' || substr($file, 1, 1) === ':') {
				self::$tplCache[$hash] = new File($file, null, $cacheId, $compileId);
			}
			elseif (is_string($this->template_dir) || is_array($this->template_dir)) {
				self::$tplCache[$hash] = new File($file, null, $cacheId, $compileId, $this->template_dir);
			}
			else {
